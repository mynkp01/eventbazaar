...
const Page = () => {
  ...
  const fetchQuote = async (
    ...
    try {
      ...
      let query = `sort_type=${sort.sort_type}&sort=${sort.sort}&page=${currentPage}&search=${keyword}&event_time=${dropDownValues?.event_time}&venue=${dropDownValues?.venue}&event_vertical=${dropDownValues?.event_vertical}&list_type=${QUOTE_TYPE.VENUE}`;
  ...
  const handleExcel = async (start_date = null, end_date = null) => {
    try {
      dispatch(setIsLoading(true));
      let query = `sort_type=${sort.sort_type}&sort=${sort.sort}&search=${searchKeys.search}&event_time=${dropDownValues?.event_time}&venue=${dropDownValues?.venue}&event_vertical=${dropDownValues?.event_vertical}&list_type=${QUOTE_TYPE.VENUE}`;
      if (!isEmpty(start_date)) {
        query += `&start_date=${dayjs(filterByDate?.start_date).format(
          "YYYY-MM-DD",
        )}`;
    ...
    <>
      <div className="border-wh-300 flex flex-col !gap-4 rounded-2xl border bg-white !p-4 md:!p-6">
        <div className="flex justify-between">
          <h1 className="text-xl font-bold">Request Quote for Venue</h1>
          <button
            key="handleReset"
            onClick={() => handleExcel()}
