...
const Page = () => {
  const eventVerticalColumns = [
    {
      name: "Event Vertical Name",
      key: "event_vertical_name",
      sort: "event_vertical_name",
    },
    {
      name: "Total Vendor",
      key: "totalVendor",
  ...
  const [eventVerticalData, setEventVerticalData] = useState([]);
  ...
  const fetchEventVerticals = async (currentPage, keyword = "") => {
    try {
      dispatch(setIsLoading(true));
      const query = `sort_type=${sort.sort_type}&sort=${sort.sort}&page=${currentPage}&search=${keyword}`;
      const { data, status } = await apiHandler.eventVertical.list(query);

      if (status === 200 || status === 201) {
        const formattedData = data?.data?.data;

        setEventVerticalData(formattedData);
  ...
  const handleExcel = async (keyword = "") => {
    try {
      ...
      const { data, status } = await apiHandler.eventVertical.excel(query);
      if (status === 200 || status === 201) {
        await download(
          data?.data,
  ...
  const handleView = (eventVertical) => {
    // router.push(
    //   `${ROUTES.admin.eventVertical}/option?id=${eventVertical._id}&view=1`,
    // );
    window.open(
      `${ROUTES.admin.eventVertical}/option?id=${eventVertical._id}&view=1`,
      "_blank",
  ...
  const handleUpdate = (eventVertical) => {
    // router.push(`${ROUTES.admin.eventVertical}/option?id=${eventVertical._id}`);
    window.open(
      `${ROUTES.admin.eventVertical}/option?id=${eventVertical._id}`,
      "_blank",
      "noopener,noreferrer",
    );
  };
  ...
  useEffect(() => {
    fetchEventVerticals(page, searchKeyword);
  ...
  return (
    <div className="border-wh-300 flex flex-col !gap-4 rounded-2xl border bg-white !p-4 md:!p-6">
      <div className="flex justify-between">
        <h1 className="text-xl font-bold">Manage Event Verticals</h1>
        <button
          key="handleExcel"
          onClick={() => handleExcel()}
        ...
        {/* {pagePermissions?.[PERMISSIONS.CREATE] ? (
          <button
            onClick={() => router.push(`${ROUTES.admin.eventVertical}/option`)}
      ...
      <Table
        columns={eventVerticalColumns}
        data={eventVerticalData}
        actions={{
