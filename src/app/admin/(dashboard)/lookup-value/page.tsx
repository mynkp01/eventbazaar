...
const Page = () => {
  ...
  const handleView = (lookup) => {
    ...
    window.open(
      `${ROUTES.admin.lookupValue}/option?id=${lookup._id}&view=1`,
      "_blank",
      "noopener,noreferrer",
    );
  };
  ...
  const handleUpdate = (lookup) => {
    ...
    window.open(
      `${ROUTES.admin.lookupValue}/option?id=${lookup._id}`,
      "_blank",
      "noopener,noreferrer",
    );
  };
  ...
  useEffect(() => {
    fetchLookups(page, searchKeys.search);
  ...
  const filters = [
    ...
    <div
      key="dropDownValues.category_id"
      ...
      <div className="flex justify-between">
        ...
        {pagePermissions?.[PERMISSIONS.CREATE] ? (
          <button
            onClick={() =>
              window.open(
                `${ROUTES.admin.lookupValue}/option`,
                "_blank",
                "noopener,noreferrer",
              )
            }
