  ...
  const fetchArtist = async () => {
    try {
      ...
      const response = await apiHandler.category.getCategoryWiseVendorList(
        `page=${currentPage}&limit=${limit}&location_id=${selectedCity?._id}&business_sub_category_ids=${businessSubCategory?.join(",")}&user_id=${userData?.user_id}`,
      );
    ...
    try {
      const response =
        await apiHandler.businessSubCategory.getBusinessSubCategoryFilterList(
          `page=${currentPage}&location_id=${selectedCity?._id}`,
        );
      if (response.status === 200) {
        const { data } = response.data;
        setFilterData(data);
        ...
        <div className="flex w-full gap-5">
          ...
          <div className="relative flex h-full w-full flex-col gap-4">
            <div className="flex w-full items-center justify-between gap-4">
              <div>
                <Breadcrumb />
                <div className="text-[38px] font-medium capitalize">Artist</div>
              </div>
                  ...
                  <div className="grid grid-cols-1 gap-8 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                    {artistData?.map((item) => (
                      <VendorDetailsBox
                        ...
                        href={`${ROUTES.client.artist}/${customEncodeURIComponent(item?.company_name)}-${item?._id}`}
