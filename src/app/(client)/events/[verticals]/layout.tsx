...
export async function generateMetadata({ params }): Promise<Metadata> {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_LOCAL}${Query.metaLookup}?keyword=events&search=${customDecodeURIComponent(params?.verticals)}`,
      {
        cache: "no-store",
      },
    );
    ...
    console.log(`error from meta lookup in ${params?.verticals}: ${e}`);
  }
}
