...
export async function generateMetadata(
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_LOCAL}${Query.metaLookup}?keyword=category&search=${customDecodeURIComponent(params?.category)}`,
      {
        cache: "no-store",
      },
    );
    ...
    console.log(`error from meta lookup in ${params?.category}: ${e}`);
  }
}
